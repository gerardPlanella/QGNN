...
class QBP(GenericTNModel):
    ...
    def forward(self, z, edge_index_out):
...
class QGNN_Layer(nn.Module):
    def __init__(self, params):
        ...
        self.z_net = nn.Sequential(
            nn.Linear(self.phys_dim*2 + 2*(2*(2**self.phys_dim)), self.hidden_dim_z, dtype=self.dtype),
            nn.SiLU(),
    ...
    def forward(self, z, edge_index):
        ...
        onerdms, twordms = self.QBP(z, edge_index)
        ...
        states = torch.cat((onerdms, twordm_aggr), dim=1)
...
class QGNN(nn.Module):
    ...
    def forward(self, datapoint, format_output=False):
        ...
        for i, layer in enumerate(self.layers):
            _z, onerdms, twordms = layer(z, edge_index)
