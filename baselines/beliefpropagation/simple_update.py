...
class SimpleUpdate:
    ...
    """
    def __init__(self, tensor_network: TensorNetwork, j_ij: list, h_k: list, s_i: list, s_j: list,
        ...
        :param h_k: The "field" constant coefficients
    ...
    def get_particle_indices(self, edge):
        which_tensors = np.nonzero(self.structure_matrix[:, edge])[0]
        assert len(which_tensors) == 2
        return which_tensors[0], which_tensors[1]
    
    def reduce_hamiltonian(self, hamiltonian, n1_index, n2_index, i_spin_dim, j_spin_dim):
        N = np.log2(hamiltonian.shape[0]).astype(int)       
        # Calculate the dimension of the reduced Hamiltonian
        dim_reduced = i_spin_dim * j_spin_dim

        # Initialize the reduced Hamiltonian
        reduced_hamiltonian = np.zeros((i_spin_dim * j_spin_dim, i_spin_dim * j_spin_dim), dtype=complex)

        # Calculate the size of a single qubit's Hilbert space
        dim_single_qubit = i_spin_dim

        # Iterate over all basis states of the reduced Hamiltonian
        for i in range(dim_reduced):
            for j in range(dim_reduced):
                # Decompose the multi-index into individual qubit indices
                i_indices = [(i >> k) & 1 for k in range(2)]
                j_indices = [(j >> k) & 1 for k in range(2)]

                # Sum over the states of the other qubits
                for k in range(2 ** (N - 2)):
                    # Build the multi-index for the original Hamiltonian
                    full_index_i = 0
                    full_index_j = 0
                    ...
                    for qubit in range(N):
                        if qubit == n1_index:
                            full_index_i |= i_indices[0] << qubit
                            full_index_j |= j_indices[0] << qubit
                        elif qubit == n2_index:
                            full_index_i |= i_indices[1] << qubit
                            full_index_j |= j_indices[1] << qubit
                        else:
                            full_index_i |= ((k >> next_index) & 1) << qubit
                            full_index_j |= ((k >> next_index) & 1) << qubit
                            next_index += 1

                    # Add the contribution to the reduced Hamiltonian
                    reduced_hamiltonian[i, j] += hamiltonian[full_index_i, full_index_j]

                # Normalize by the number of summed states
                reduced_hamiltonian[i, j] /= 2 ** (N - 2)
    ...
    def get_hamiltonian(self, i_neighbors, j_neighbors, ek):
        ...
        if self.hamiltonian is not None:
            n1_index, n2_index = self.get_particle_indices(ek)
            # Create Local Hamiltonian from Full Hamiltonian
            local_hamiltonian = self.reduce_hamiltonian(self.hamiltonian, n1_index, n2_index, self.s_i[0].shape[0], self.s_j[0].shape[0])    
        ...
        # Create the local Hamiltonian from spin operators and Jij, hi constants
            ...
            for i, _ in enumerate(self.s_i):
                interaction_hamiltonian += np.kron(self.s_k[i], self.s_k[i])
            ...
            # Determine the indices for particles i and j based on ek
            n1_index, n2_index = self.get_particle_indices(ek)

            # Construct the Hamiltonian #TODO: Needed? (normalize the field operators according to the number of neighbors)
            # hamiltonian = self.j_ij[ek] * interaction_hamiltonian + (
            #     self.h_k[n1_index][0] * i_field_hamiltonian  + 
            #     self.h_k[n2_index][0] * j_field_hamiltonian
            # )

            hamiltonian = self.j_ij[ek] * interaction_hamiltonian + (
                self.h_k[n1_index][0] * i_field_hamiltonian  +
                self.h_k[n1_index][1] * j_field_hamiltonian +
                self.h_k[n2_index][0] * i_field_hamiltonian  +
                self.h_k[n2_index][1] * j_field_hamiltonian
            )
